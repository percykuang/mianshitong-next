...
export function isChatModelId(value: string): value is ChatModelId {
  return value in CHAT_MODEL_BY_ID
}
...
export function normalizeChatModelId(
  ...
  return value && isChatModelId(value) ? value : DEFAULT_CHAT_MODEL_ID
}

// 根据模型 id 获取它在底层 provider 中对应的角色。
export function getChatModelRole(modelId: ChatModelId): ModelRole {
  return CHAT_MODEL_BY_ID[modelId].role
}

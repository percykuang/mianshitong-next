...
export function renderInlineCode({
  ...
  return (
    <code
      className={`rounded-md bg-slate-900/6 px-1.5 py-0.5 font-mono text-[0.9em] wrap-break-word whitespace-break-spaces text-(--mst-color-primary) dark:bg-white/8 ${className ?? ''}`.trim()}
      {...elementProps}
    >
      {children}
    </code>
